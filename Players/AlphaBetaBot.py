...
class AlphaBetaBot(BotInterface):
    name = "AlphaBeta Bot"
    ...
    def makeTheMove(self, movesMade):
        ...
        whenToStop = (depth - (totalMoves // depth))
        # whenToStop =

        # print("Depth:" + str(depth))
        # print("Bot is stopping in:" + str(whenToStop))
        ...
        move = self.minimax(movesMadeNew, depth, 1, whenToStop, 0, 0, time.time(), -self.infinity, self.infinity)
        bestmove = move[0]
        score = move[1]
    ...
    def minimax(self, state, depth, player, whenToStop, opponent1_score_before, opponent2_score_before, startTime, alfa,
                beta):
        if player == 1:
        ...
        for move in self.getFreeLines(state):
            move["who"] = player
            # print(move)
            # opponent1_score_old, opponent2_score_old = self.evaluate(state)
            state.append(move)
            opponent1_score, opponent2_score = self.evaluate(state)
            # we need to check if this the score has changed and if yes
            # we dont need to change the turn
            #

            if opponent1_score_before != opponent1_score or opponent1_score_before != opponent2_score:
                nextturn = player
            else:
                nextturn = -1 * player
